...
--gold.dim_customers
	...
	select
		row_number() over (order by cst_id) as customer_key,--surogate key, standarizes the key to k for k in {1,2,..}
		...
		cast(year(getdate()) as int)-cast(year(bt.bdate) as int) as age, --derived age column; only takes year from bdate 
...
--gold.dim_products
	...
	select 
		row_number() over (order by i.prd_start_dt ,i.prd_key) as product_key,--surogate key, standarizes the key to k for k in {1,2,..}
